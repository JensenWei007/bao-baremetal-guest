...
void clock_init(void) {
    ...
    clk_ctl_iso = (struct clk_ctl_iso_hw*)PLAT_CLK_ISO_BASE;
    clk_ctl_awo = (struct clk_ctl_awo_hw*)PLAT_CLK_AWO_BASE;

    /* Disable CLK registers protection */
    clk_ctl_iso->CLKKCPROT1 = CLKKCPROT1_ENABLE_WR;

    /* Start main oscillator, if not started */
    if (!(clk_ctl_awo->MOSCS & MOSCS_MOSCEN_BIT) &&
    ...
    if (!(clk_ctl_iso->PLLS & PLLS_PLLCLKEN_BIT) &&
